...
export const CrmSidebar = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <aside className={`bg-navy-950 border-r border-navy-800 text-slate-300 flex flex-col justify-between transition-all duration-300 z-30 ${collapsed ? 'w-20' : 'w-64'}`}>
      
      {/* Top Brand Logo */}
      <div className="p-4 border-b border-navy-800 flex items-center justify-between">
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveModule('dashboard')}>
          <div className="w-9 h-9 rounded-xl bg-navy-900 border-2 border-gold-500 flex items-center justify-center shadow-lg shrink-0">
          ...
          {!collapsed && (
            <div>
              <h2 className="font-serif text-base font-bold text-white tracking-tight leading-none">SIR CRM</h2>
              <span className="text-[9px] uppercase tracking-widest text-gold-400 font-bold">Enterprise Edition</span>
            </div>
          )}
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl transition duration-200 ${
                isActive
                  ? 'bg-gold-500/20 text-gold-400 border border-gold-500/40 font-bold shadow-sm'
                  : 'hover:bg-navy-900 hover:text-white text-slate-400'
              }`}
              title={item.label}
            >
              <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-gold-400' : 'text-slate-400'}`} />
      ...
      {/* User Profile & Logout */}
      <div className="p-3 border-t border-navy-800 bg-navy-900/50">
        {!collapsed ? (
          <div className="flex justify-between items-center text-xs">
            <div className="flex items-center space-x-2.5 truncate">
              ...
              <div className="truncate">
                ...
                <p className="text-[10px] text-gold-400 font-semibold truncate">{currentRole}</p>
              </div>
            </div>
            <button onClick={logout} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-navy-800" title="Logout">
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        ) : (
          <button onClick={logout} className="w-full flex justify-center p-2 text-slate-400 hover:text-rose-400">
