        ...
        <div className="md:col-span-7 p-6 sm:p-8 bg-slate-50/50 dark:bg-navy-900/60 flex flex-col justify-center">
          ...
          {authMode === 'login' ? (
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setRegData({ ...regData, fullName: e.target.value.replace(/[^a-zA-Z\s\.\'-]/g, '') })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-700 dark:text-slate-300 font-semibold">Phone Number (Numbers Only)</label>
                  <div className="flex space-x-1.5">
                    <select
                      value={regData.countryCode}
                      onChange={(e) => {
                        const newCode = e.target.value;
                        const maxD = newCode === '+91' || newCode === '+44' ? 10 : newCode === '+974' ? 8 : 9;
                        setRegData({ ...regData, countryCode: newCode, phone: regData.phone.replace(/\D/g, '').slice(0, maxD) });
                      }}
                      className="bg-white dark:bg-navy-950 border border-slate-300 dark:border-navy-700 rounded-xl px-2 py-2 text-slate-900 dark:text-white text-xs font-bold"
                    >
                      <option value="+971">🇦🇪 +971 (UAE - 9 digits)</option>
                      <option value="+966">🇸🇦 +966 (KSA - 9 digits)</option>
                      <option value="+91">🇮🇳 +91 (India - 10 digits)</option>
                      <option value="+974">🇶🇦 +974 (Qatar - 8 digits)</option>
                      <option value="+44">🇬🇧 +44 (UK - 10 digits)</option>
                    </select>
                    <input
                      type="text"
                      inputMode="numeric"
                      ...
                      value={regData.phone}
                      onChange={(e) => {
                        const maxD = regData.countryCode === '+91' || regData.countryCode === '+44' ? 10 : regData.countryCode === '+974' ? 8 : 9;
                        const digitsOnly = e.target.value.replace(/\D/g, '').slice(0, maxD);
